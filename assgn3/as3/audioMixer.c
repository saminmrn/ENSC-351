...
enum Interval_whichInterval audioBufferInterval = INTERVAL_LOW_LEVEL_AUDIO;
Interval_statistics_t pAudio;
...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	if (soundBite_index == MAX_SOUND_BITES + 1 && soundBites[soundBite_index].pSound != NULL)
	{
		printf("there is no free slot in the sound array");
		return;
	}
	// adding sound to the queue
	soundBites[soundBite_index].pSound = pSound;
	soundBites[soundBite_index].location = 0;
...
int AudioMixer_get_BPM()
{
	return bpm;
}
void AudioMixer_set_BPM(int newBPM)
{
	if (newBPM < MIN_BPM)
	{
		newBPM = MIN_BPM;
...
double wait_for_halfBeat()
{
	double halfBeat = ((double)60 / AudioMixer_get_BPM() / 2) * 1000;
	return halfBeat;
}
...
static void fillPlaybackBuffer(short *buff, int size)
{
	...
	int i = 0;
	// looping through soundbite array
	while (i < MAX_SOUND_BITES)
	{
		if (soundBites[i].pSound != NULL)
		{
			...
			int j = 0;
			int play_offset = soundBites[i].location;

			while (play_offset < soundBites[i].pSound->numSamples && j < playbackBufferSize)
			{

				int data_offset = (int)soundBites[i].pSound->pData[play_offset];
				// adding data
				int data = ((int)buff[j]) + data_offset;
				// clipping
				if (data > SHRT_MAX)
				{
					data = SHRT_MAX;
			...
			soundBites[i].location = play_offset;
	...
	Interval_markInterval(audioBufferInterval);
	pthread_mutex_unlock(&audioMutex);
}
...
void *playbackThread(void *_arg)
{
	while (!stopping)
	{
		...
		// Interval_markInterval(audioBufferInterval); // making interval
		// Interval_getStatisticsAndClear(audioBufferInterval,pAudio); // get interval data
...
void setStats()
{
	Interval_getStatisticsAndClear(audioBufferInterval, &pAudio);
}

int getSamples()
{
	return pAudio.numSamples;
}

double getAverage()
{
	return pAudio.avgIntervalInMs;
}

double getMin()
{
	return pAudio.minIntervalInMs;
}

double getMax()
{
	return pAudio.maxIntervalInMs;
}